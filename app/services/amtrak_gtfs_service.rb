...
class AmtrakGtfsService
  ...
  def import
    ...
    begin
      ...
      # Create departures for the next 24 hours
      create_departures(routes, stops, trips, stop_times, stats)

      Rails.logger.info "[TransitTracker] Amtrak import complete: #{stats}"
    rescue => e
      Rails.logger.error "[TransitTracker] Amtrak import failed: #{e.message}"
  ...
  def download_and_extract_gtfs
    Rails.logger.info "[TransitTracker] Downloading Amtrak GTFS from #{GTFS_URL}"

    tempfile = Tempfile.new(["amtrak_gtfs", ".zip"], binmode: true)
  ...
  def build_schedule_content(stops, route, trip_data)
    ...
    content += "**Route:** #{route[:short_name] || route[:long_name]}\n"
